...
TOOL_DECLARATIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Search the web for a query, automatically open the top matching URL "
            "(such as a Reddit post, blog, or documentation page), and extract its text content."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "The search query (e.g., 'python asyncio best practices reddit').",
                }
            },
            "required": ["query"],
        },
    ...
    {
        "name": "read_clipboard",
        "description": "Read the text currently stored in the system clipboard.",
        "parameters": {"type": "object", "properties": {}},
    },
    {
    ...
    },
    {
        ...
        "description": "Read the text or code content of a specified local file path.",
        "parameters": {
            "type": "object",
            "properties": {
...
def run_setup_wizard_step1() -> tuple[str, str] | None:
    ...
    root = tk.Tk()
    root.title(f"{APP_NAME} - First Time Setup (1/3: API & Model)")
    ...
    btn = tk.Button(root, text="Next Step", command=on_confirm, bg="#0078D7", fg="white", width=16)
    btn.pack(anchor="e")
...
def run_setup_wizard_step2() -> str:
    ...
    root = tk.Tk()
    root.title(f"{APP_NAME} - First Time Setup (2/3: Keybinds)")
    ...
    btn = tk.Button(root, text="Next Step", command=on_save, bg="#0078D7", fg="white", width=16)
    btn.pack(anchor="e")

    root.mainloop()
    return hotkey_holder["hotkey"]

def run_setup_wizard_step3() -> str:
    """Wizard Step 3: Configure custom AI behavior/personality."""
    import tkinter as tk

    personality_holder = {"personality": ""}

    root = tk.Tk()
    root.title(f"{APP_NAME} - First Time Setup (3/3: AI Behavior)")
    root.geometry("460x250")
    root.resizable(False, False)
    root.configure(padx=20, pady=20)

    tk.Label(root, text="Custom AI Behavior", font=("Arial", 14, "bold")).pack(anchor="w", pady=(0, 5))
    tk.Label(
        root,
        text="Step 3: Define optional custom tone or instructions.\nCore system safety rules remain permanently active.",
        font=("Arial", 9),
        justify="left"
    ).pack(anchor="w", pady=(0, 15))

    tk.Label(root, text="Additional Persona / Instructions (Optional):", font=("Arial", 9, "bold")).pack(anchor="w")
    personality_entry = tk.Entry(root, width=52)
    personality_entry.pack(anchor="w", pady=(5, 20))
    personality_entry.focus_set()

    def on_save():
        val = personality_entry.get().strip()
        personality_holder["personality"] = val
        root.destroy()

    btn = tk.Button(root, text="Finish & Launch", command=on_save, bg="#0078D7", fg="white", width=16)
    btn.pack(anchor="e")
...
class AssistantSession:
    ...
    def ask(self, message: str) -> str:
        """Send one user message, return Gemini's response, and save history."""
        self.history.append(
        ...
        try:
            ...
            log_debug(f"[HISTORY ERROR] Could not save session: {e}")
...
def bring_to_foreground(hwnd: int) -> None:
    """Robustly force a window to the foreground on Windows."""
    if sys.platform != "win32":
        return
    try:
...
def launch_chat_ui(client: Any, model: str = DEFAULT_MODEL) -> None:
    ...
    def open_settings_dialog():
        ...
        tk.Label(settings_win, text="Personality & Tone (Optional):", font=("Arial", 9, "bold")).pack(anchor="w")
        personality_entry = tk.Entry(settings_win, width=52)
        personality_entry.insert(0, app_cfg.get("personality", ""))
        ...
        def save_settings():
            ...
            # Update current session config dynamically
...
def main() -> int:
    ...
    personality = app_config.get("personality", "")
    ...
    if not api_key or arguments.firstboot:
        ...
        personality = run_setup_wizard_step3()
        save_app_config({"model": model, "hotkey": hotkey, "personality": personality})
